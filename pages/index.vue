        ...
        <template v-if="isMobile && page[0].data.slices[2] && page[0].data.slices[2].slice_type == 'mobile_slider'">
          <div class="relative md:w-[56vw] m-auto">
              ...
              <VueSlickCarousel ref="carousel" :arrows="false" :adaptiveHeight="true" :autoplaySpeed="5000"
                ...
                <div v-for="(item, i) in page[0].data.slices[2].items" :key="`slice-item-${i}`" class="m-auto pt-1.5">
                  <div class="relative">
                    ...
                    <div
                      v-if="page[0].data.status != 'ORDER' && page[0].data.blurring == 'all slides' || page[0].data.status != 'ORDER' && page[0].data.blurring == 'only first slide' && i == 0"
                      :class="page[0].data.status == 'email' ? '' : 'preorder'"
                      class="absolute inset-0 backdrop-brightness-130 backdrop-blur-lg bg-white/60 flex justify-center items-center text-2xl font-cooperbt text-[#6200FF]">
                      <div v-if="page[0].data.status == 'email'" class="opacity-60">「out of stock」</div>
                      <div v-else-if="page[0].data.status == 'PRE-ORDER' || item.data.status == 'customize'"
                        class="opacity-60 flex flex-col text-center gap-[23px]">「{{ page[0].data.purple_text[0].text }}」
                        <div class="text-lg indexcounter pt-[7px]"></div>
                        <div class="text-lg font-garamond">
            ...
            <div v-if="page[0].data.slices[0].items.length > 1"
              class="hidden md:flex justify-between px-4 h-12 w-full -translate-y-1/2 absolute top-1/2"> <button
                ...
                <div v-for="(item, i) in page[0].data.slices[0].items" :key="`slice-item-${i}`" class="m-auto pt-1.5">
                  <div class="relative">
                    ...
                    <div
                      ...
                      class="absolute inset-0 backdrop-brightness-130 backdrop-blur-lg bg-white/60 flex justify-center items-center text-2xl font-cooperbt text-[#6200FF]">
                      <div v-if="page[0].data.status == 'email'" class="opacity-60">「out of stock」</div>
                      <div v-else-if="page[0].data.status == 'PRE-ORDER' || page[0].data.status == 'customize'"
                        class="opacity-60  flex flex-col text-center gap-[23px]">「{{ page[0].data.purple_text[0].text
                        }}」<div class="text-lg indexcounter pt-[7px]"></div>
                        <div class="text-lg font-garamond">
                          <PrismicRichText :field="page[0].data.preordertext" />
